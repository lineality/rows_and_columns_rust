...
///
/// This function creates a new report file and writes the overview/summary section
/// including file metadata, basic structure information, and column type counts.
/// The file is created in the same directory as the source CSV file.
///
/// # Arguments
/// * `csv_file_path` - Absolute path to the analyzed CSV file
/// * `analysis_results` - Basic CSV structure and column information
/// * `directory_paths` - Application directory paths for storage references
///
/// # Returns
/// * `RowsAndColumnsResult<PathBuf>` - Path to the created report file or error
///
/// # Errors
/// * `RowsAndColumnsError::FileSystemError` - If file creation or writing fails
/// * `RowsAndColumnsError::ConfigurationError` - If paths cannot be determined
pub fn save_analysis_summary_to_file(
    ...
    let csv_parent_directory = csv_file_path.parent()
        .ok_or_else(|| RowsAndColumnsError::ConfigurationError {
            configuration_issue_description: "Cannot determine parent directory of CSV file".to_string(),
        })?;

    // Get CSV filename without extension for report naming
    let csv_stem = csv_file_path.file_stem()
        .and_then(|stem| stem.to_str())
        .ok_or_else(|| RowsAndColumnsError::ConfigurationError {
            configuration_issue_description: "Cannot determine CSV filename".to_string(),
        })?;

    // Create timestamp string (simple format without external crates)
    let timestamp = format!("{:?}", std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .map_err(|e| RowsAndColumnsError::ConfigurationError {
            configuration_issue_description: format!("System time error: {}", e),
        })?
        .as_secs());

    // Build report filename
    let report_filename = format!("{}_analysis_report_{}.toml", csv_stem, timestamp);
    let report_file_path = csv_parent_directory.join(report_filename);

    // Get file size if available
    let file_size_bytes = std::fs::metadata(csv_file_path)
    ...
    for column_info in &analysis_results.column_information_list {
        match column_info.detected_data_type {
            CsvColumnDataType::Integer | CsvColumnDataType::Float => continuous_count += 1,
            CsvColumnDataType::Boolean | CsvColumnDataType::String => categorical_count += 1,
        }
    }
    ...
    // Metadata section
    toml_content.push_str("[metadata]\n");
    toml_content.push_str(&format!("timestamp = \"{}\"\n", timestamp));
    toml_content.push_str(&format!("original_csv_file = \"{}\"\n", csv_file_path.display()));
    toml_content.push_str("report_generated_by = \"rows_and_columns\"\n");
    toml_content.push_str("\n");

    // Summary section
    toml_content.push_str("[summary]\n");
    toml_content.push_str(&format!("file_size_bytes = {}\n", file_size_bytes));
    toml_content.push_str(&format!("total_data_rows = {}\n", analysis_results.total_data_row_count));
    toml_content.push_str(&format!("total_columns = {}\n", analysis_results.total_column_count));
    toml_content.push_str(&format!("continuous_columns = {}\n", continuous_count));
    toml_content.push_str(&format!("categorical_columns = {}\n", categorical_count));
    toml_content.push_str(&format!("has_header_row = {}\n", analysis_results.has_header_row));
    toml_content.push_str("\n");
    ...
    std::fs::write(&report_file_path, toml_content)
        .map_err(|io_error| create_file_system_error(
            &format!("Failed to write analysis report to {}", report_file_path.display()),
            io_error
        ))?;
...
/// Appends detailed column-by-column analysis to the TOML report file
///
/// This function adds detailed statistical analysis for each column including
/// numerical statistics (min, max, quartiles, mean, stdev) for continuous columns
/// and categorical statistics (unique values, mode, value distributions) for categorical columns.
///
/// # Arguments
/// * `report_file_path` - Path to the existing report file to append to
/// * `enhanced_analysis_results` - Vector of enhanced column information with statistics
///
/// # Returns
/// * `RowsAndColumnsResult<()>` - Success or error
///
/// # Errors
/// * `RowsAndColumnsError::FileSystemError` - If file reading or writing fails
pub fn save_analysis_details_to_file(
    report_file_path: &PathBuf,
    enhanced_analysis_results: &Vec<EnhancedCsvColumnInformation>,
) -> RowsAndColumnsResult<()> {
    // Read existing content
    let mut toml_content = std::fs::read_to_string(report_file_path)
        .map_err(|io_error| create_file_system_error(
            &format!("Failed to read existing report file {}", report_file_path.display()),
            io_error
        ))?;

    // Add columns header if we have columns to report
    if !enhanced_analysis_results.is_empty() {
        toml_content.push_str("[columns]\n");
        toml_content.push_str("\n");
    }

    // Process each column
    for column_info in enhanced_analysis_results {
        let column_name = &column_info.basic_info.column_name;
        let safe_column_name = column_name.replace(" ", "_").replace(".", "_");

        // Column header
        toml_content.push_str(&format!("[columns.{}]\n", safe_column_name));

        // Basic information
        toml_content.push_str(&format!("column_index = {}\n", column_info.basic_info.column_index));
        toml_content.push_str(&format!("column_name = \"{}\"\n", column_name));
        toml_content.push_str(&format!("data_type = \"{:?}\"\n", column_info.basic_info.detected_data_type));
        toml_content.push_str(&format!("field_type = \"{:?}\"\n", column_info.field_type));
        toml_content.push_str(&format!("non_empty_values = {}\n", column_info.basic_info.non_empty_value_count));
        toml_content.push_str(&format!("empty_values = {}\n", column_info.basic_info.empty_value_count));

        // Add numerical statistics if continuous
        if let Some(ref num_stats) = column_info.numerical_statistics {
            toml_content.push_str(&format!("min_value = {:.3}\n", num_stats.min_value));
            toml_content.push_str(&format!("q1_value = {:.3}\n", num_stats.q1_value));
            toml_content.push_str(&format!("q2_median_value = {:.3}\n", num_stats.q2_median_value));
            toml_content.push_str(&format!("q3_value = {:.3}\n", num_stats.q3_value));
            toml_content.push_str(&format!("max_value = {:.3}\n", num_stats.max_value));
            toml_content.push_str(&format!("mean_value = {:.3}\n", num_stats.mean_value));
            toml_content.push_str(&format!("standard_deviation = {:.3}\n", num_stats.standard_deviation));
            toml_content.push_str(&format!("missing_percentage = {:.1}\n", num_stats.missing_percentage));
        }

        // Add categorical statistics if categorical
        if let Some(ref cat_stats) = column_info.categorical_statistics {
            toml_content.push_str(&format!("unique_value_count = {}\n", cat_stats.unique_value_count));
            toml_content.push_str(&format!("missing_percentage = {:.1}\n", cat_stats.missing_percentage));

            // Mode information
            if let Some(ref mode) = cat_stats.mode_value {
                toml_content.push_str(&format!("mode_value = \"{}\"\n", mode));
                toml_content.push_str(&format!("mode_percentage = {:.1}\n", cat_stats.mode_percentage));
            }

            // Value distribution (limit to avoid huge files)
            toml_content.push_str("value_distribution = [\n");
            let max_values_to_show = 20; // Reasonable limit for report
            for (index, freq) in cat_stats.value_frequencies.iter().enumerate() {
                if index >= max_values_to_show {
                    toml_content.push_str(&format!("  # ... and {} more values\n",
                        cat_stats.value_frequencies.len() - max_values_to_show));
                    break;
                }
                toml_content.push_str(&format!(
                    "  {{ value = \"{}\", count = {}, percentage = {:.1} }},\n",
                    freq.value.replace("\"", "\\\""), // Escape quotes in values
                    freq.count,
                    freq.percentage
                ));
            }
        ...
        // Sample values
        toml_content.push_str("sample_values = [");
        for (index, sample) in column_info.basic_info.sample_values.iter().enumerate() {
            if index > 0 {
                toml_content.push_str(", ");
            }
            toml_content.push_str(&format!("\"{}\"", sample.replace("\"", "\\\"")));
        }
        toml_content.push_str("]\n");
    ...
    std::fs::write(report_file_path, toml_content)
        .map_err(|io_error| create_file_system_error(
            &format!("Failed to write detailed analysis to {}", report_file_path.display()),
            io_error
        ))?;
